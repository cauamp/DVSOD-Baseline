...
class SalObjDataset(data.Dataset):
    def __init__(self, data_root, subset, augmentation, interval, sample_rate, trainsize, baseMode):
        ...
        for video in videolists:
            ...
            image_list = [x for x in os.listdir(image_path) if x.endswith(".jpg") or x.endswith(".png")]
            file_postfix = image_list[0][-4:] if image_list else ".png"
            filenames_i = [
                os.path.join(image_path, f[:-4] + file_postfix)
                for f in os.listdir(label_path)
                if any(f.endswith(ext) for ext in [".jpg", ".png"])
            ]
            ...
            depth_list = [x for x in os.listdir(depth_path) if x.endswith(".jpg") or x.endswith(".png")]
            file_dep_postfix = depth_list[0][-4:] if depth_list else ".png"
            filenames_dep_i = [
                os.path.join(depth_path, f[:-4] + file_dep_postfix)
                for f in os.listdir(label_path)
                if any(f.endswith(ext) for ext in [".jpg", ".png"])
            ]
    ...
    def filename_from_index(self, base_file_path, index):
        ...
        new_path = os.path.join(os.path.dirname(base_file_path), new_file_name)
